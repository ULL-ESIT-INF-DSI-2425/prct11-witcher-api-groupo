...
describe("Merchant API", () => {
  ...
  it("Should find an existing merchant by id", async () => {
    const example = await request(app).post("/merchants").send({
      id: 13,
  ...
  it("Should send an error for not providing a body", async () => {
    ...
    await request(app).patch(`/merchants/${response._id}`).send({}).expect(400);
  });
  ...
  it("Should successfully delete an existing merchant", async () => {
    await request(app).delete(`/merchants?name=MercaderPrueba`).expect(200);
  });

  it("Should unsuccessfully delete an inexisting merchant", async () => {
    await request(app).delete("/merchants?name=Merchant of Rivia").expect(404);
  });
  ...
  it("Should successfully delete an existing merchant", async () => {
    ...
    await request(app).delete(`/merchants/${response._id}`).expect(200);
  });
